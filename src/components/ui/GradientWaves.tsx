...
const detailToSteps = (detail: GradientWavesDetail): number => {
  ...
  if (detail === 'high') return 64.0;
  return 44.0;
};

/**
 * Resolution scale for the shader canvas.
 *
 * This is a fullscreen sphere-tracer: every pixel walks the SDF up to `uSteps`
 * times, so cost is literally `pixels × steps`. At the previous `dpr: min(dPR,
 * 2)` a 1920×1080 window on a HiDPI display rendered 3840×2160 — 8.3M pixels ×
 * 70 steps ≈ 580M plasma evaluations per frame, every frame, forever. That is
 * the single largest reason the app stalled: it starved the GPU that the rest
 * of the UI needs for compositing, so scrolling and typing went to treacle.
 *
 * The output is a soft, low-frequency gradient with no fine detail to lose, so
 * rendering it below device resolution and letting the browser scale the canvas
 * up is visually free. 0.65 was picked by looking at it.
 */
const RENDER_SCALE = 0.65;

/** Frame budget. 30fps is plenty for a slow-moving backdrop and halves the
 *  work compared with letting it run at the display's refresh rate. */
...
const GradientWaves: React.FC<GradientWavesProps> = ({
  ...
  useEffect(() => {
    ...
    /* Tracked on the window, not on the canvas.
       The container sits at `z-index: -10` behind the whole app, so a listener
       bound to the canvas only ever fired in the gaps between UI elements —
       the parallax worked in patches and died the moment the pointer crossed a
       panel. Listening on the window is both correct and cheaper: the handler
       stores two numbers and the render loop reads them at its own rate. */
    ...
    const loop = (t: number) => {
      ...
      /* Skip frames rather than render them. requestAnimationFrame fires at the
         display's refresh rate — 120Hz on some panels — and there is nothing in
         a slow swell that needs to be resolved that finely. */
      ...
      currentMouse[0] += 0.12 * (tx - currentMouse[0]);
      currentMouse[1] += 0.12 * (ty - currentMouse[1]);
  ...
  useEffect(() => {
    ...
    if (hc[0] !== h[0] || hc[1] !== h[1] || hc[2] !== h[2]) {
      hc[0] = h[0]; hc[1] = h[1]; hc[2] = h[2];
    }
    if (wc[0] !== w[0] || wc[1] !== w[1] || wc[2] !== w[2]) {
      wc[0] = w[0]; wc[1] = w[1]; wc[2] = w[2];
    }
    if (cc[0] !== cr[0] || cc[1] !== cr[1] || cc[2] !== cr[2]) {
