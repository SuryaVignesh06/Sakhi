...
const Ferrofluid: React.FC<FerrofluidProps> = ({
  ...
  useEffect(() => {
    ...
    // Cap DPR to max 0.85 for silky smooth, low-power ambient rendering
    const requestedDpr = dpr ?? (typeof window !== 'undefined' ? Math.min(window.devicePixelRatio || 1, 0.85) : 0.85);
    const renderer = new Renderer({
      dpr: Math.min(requestedDpr, 0.85),
      alpha: true,
      antialias: false
    });
    ...
    // Frame rate throttler (~35 FPS / 28ms interval for low-power, butter-smooth animation)
    let lastRenderTime = 0;
    const frameInterval = 28; // ~35 FPS target
